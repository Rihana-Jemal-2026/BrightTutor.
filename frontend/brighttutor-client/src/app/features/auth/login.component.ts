...
@Component({
              ...
              <form (ngSubmit)="onSubmitStudentReg()">
                ...
                <div class="form-row">
                  <div class="form-group">
                    ...
                    <select [(ngModel)]="studentForm.gradeLevel" name="sGrade">
                ...
                <div class="form-row">
                  <div class="form-group">
                    ...
                    <select [(ngModel)]="studentForm.desiredServiceType" name="sServiceType" (change)="onServiceTypeChange()" required>
                ...
                @if (studentForm.desiredServiceType === 3) {
                  <div class="home-gps-notice">
                    ...
                    @if (gpsCaptured()) {
                      <div class="gps-status-badge success">
                        ...
                        Exact Home GPS Recorded: Lat {{ studentForm.gpsLatitude?.toFixed(4) }}, Lng {{ studentForm.gpsLongitude?.toFixed(4) }}
                      </div>
                    } @else {
                ...
                <div class="form-group">
                  ...
                  @if (teacherCvFileName()) {
                    <div class="file-attached-badge">
                      ...
                      <span>Attached Document: <strong>{{ teacherCvFileName() }}</strong></span>
                    </div>
                  }
                </div>
  ...
  styles: [`
    ...
    .btn-gps-inline {
      white-space: nowrap; flex-shrink: 0; padding: 0 0.65rem; height: 38px;
      background: rgba(16, 185, 129, 0.12); border: 1.5px solid rgba(16, 185, 129, 0.4);
      color: #059669; border-radius: 8px; font-size: 0.8rem; font-weight: 700; cursor: pointer;
      display: flex; align-items: center; gap: 4px; transition: all 0.2s ease;
    }
    .btn-gps-inline:hover { background: rgba(16, 185, 129, 0.25); border-color: #059669; }
    .home-gps-notice {
      background: linear-gradient(135deg, rgba(16, 185, 129, 0.1) 0%, rgba(59, 130, 246, 0.1) 100%);
      border: 1.5px solid rgba(16, 185, 129, 0.4); border-radius: 10px; padding: 0.85rem 1rem; margin-bottom: 1rem;
    }
    .home-gps-notice .notice-header { display: flex; align-items: center; gap: 0.5rem; font-size: 0.88rem; font-weight: 700; color: #065f46; margin-bottom: 0.35rem; }
    .home-gps-notice p { font-size: 0.8rem; color: var(--color-text); margin: 0 0 0.65rem 0; line-height: 1.4; }
    .btn-detect-gps-banner {
      display: inline-flex; align-items: center; gap: 0.4rem; background: #059669; color: white;
      border: none; padding: 0.45rem 0.85rem; border-radius: 6px; font-size: 0.82rem; font-weight: 700; cursor: pointer;
    }
    .btn-detect-gps-banner:hover { background: #047857; }
    ...
    .kg-univ-prompt-box {
      ...
      padding: 0.85rem 1rem; border-radius: 10px; margin-bottom: 1rem; width: 100%; box-sizing: border-box;
      .field-hint { font-size: 0.76rem; color: #1e40af; margin-top: 0.35rem; display: block; }
    }
    .kg-univ-label {
      color: #1d4ed8 !important; font-weight: 700 !important; font-size: 0.85rem !important;
      display: flex !important; align-items: center !important; gap: 0.4rem !important; margin-bottom: 0.35rem !important;
    }
...
export class LoginComponent implements OnInit {
  ...
  isKgUnivSelected(): boolean {
    if (!this.selectedStudentCourseOption || this.selectedStudentCourseOption === 'OTHER') return false;
    const course = this.courses().find(c => c.id === this.selectedStudentCourseOption);
  ...
  teacherCvFileName = signal<string>('');

  onTeacherCvSelected(event: any): void {
    const file = event.target.files[0];
    if (file) {
      this.teacherCvFileName.set(file.name);
      const reader = new FileReader();
      reader.onload = (e: any) => {
        this.teacherForm.cvDocumentUrl = e.target.result;
      };
  ...
  detectGpsLocation(): void {
    if (!navigator.geolocation) {
      this.toastService.showError('Geolocation is not supported by your browser.');
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        this.studentForm.gpsLatitude = lat;
        this.studentForm.gpsLongitude = lng;
        ...
        fetch(`https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${lat}&lon=${lng}`)
          ...
          .then(data => {
            ...
            this.studentForm.address = formattedAddress;
            this.toastService.showSuccess(`📍 Traced Village/Location: ${placeName || 'GPS Captured'}`);
          })
          .catch(() => {
            this.detectingGps.set(false);
            const coordsStr = `GPS: ${lat.toFixed(5)}, ${lng.toFixed(5)}`;
            if (!this.studentForm.address) {
              this.studentForm.address = coordsStr;
            } else if (!this.studentForm.address.includes('GPS:')) {
              this.studentForm.address = `${this.studentForm.address.trim()} (${coordsStr})`;
            }
            this.toastService.showSuccess(`📍 GPS Location Captured! (Lat: ${lat.toFixed(4)}, Lng: ${lng.toFixed(4)})`);
          });
      },
      (error) => {
        this.detectingGps.set(false);
        this.toastService.showError('Could not detect exact GPS location. Please check location permissions or enter your address manually.');
      },
  ...
  onServiceTypeChange(): void {
    if (this.studentForm.desiredServiceType === 3) {
